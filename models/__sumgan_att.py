...
# Add parent directory to the sys.path to import modules
...
class Transformer(nn.Module):
    def __init__(self, input_size=1024, encoder_layers=4, attention_heads=8, epsilon=1e-5):
        ...
        self.layer_norm = nn.LayerNorm(input_size, epsilon)
...
class VideoSummarizer(nn.Module):
    def __init__(self, input_size=1024, s_encoder_layers=2, s_attention_heads=4,
        ...
        """VideoSummarizer: Selector (Transformer) + Autoencoder Transformer.
        Args
    ...
    def train(self, fold):
        ...
        for epoch in range(self.hps.epochs):
            ...
            for batch_i, key in enumerate(train_keys):
                ...
                # Losses
                loss_s_e = self.loss_recons(h_real, h_fake)
                loss_s_e += self.loss_ae(x, x_hat)
                loss_s_e += self.loss_sparsity(scores)
                loss_s_e.backward()
                nn.utils.clip_grad_norm_(self.model.video_summarizer.parameters(), 5.0)
                ...
                ###########################
                # Forward
                _, h_x = self.model.gan(x)
                _, h_x_hat = self.model.gan(x_hat.detach())
                _, h_x_hat_p = self.model.gan(x_hat_p.detach())

                # Losses
                probs_real = torch.mean(h_x, dim=0)
                probs_fake = torch.mean(h_x_hat, dim=0)
                probs_uniform = torch.mean(h_x_hat_p, dim=0)

                loss_d = self.loss_gan_discriminator(probs_real, probs_fake, probs_uniform)
                loss_d.backward()
                nn.utils.clip_grad_norm_(self.model.gan.parameters(), 5.0)
                ...
                _, h_x_hat_p = self.model.gan(x_hat_p)

                # Losses
                probs_fake_p = torch.mean(h_x_hat_p, dim=0)
                loss_c = self.loss_gan_generator(probs_fake, probs_uniform)
                loss_c += self.loss_gan_generator(probs_fake_p, probs_uniform)
                loss_c.backward()
                nn.utils.clip_grad_norm_(self.model.video_summarizer.parameters(), 5.0)
