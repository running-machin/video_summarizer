...
if ${CONVERT_VIDEO_TO_JPG}; then
  echo "Converting video to JPG from ${dir_in_video}${name_video}.${extension_video} ..."
  if ${CROP_VIDEO}; then
    ffmpeg -i ${dir_in_video}${name_video}.${extension_video} -ss ${start_time_sec} -t ${continuous_time_sec} -f image2 -vcodec mjpeg -qscale 1 -qmin 1 -qmax 1 -r ${frame_rate} ${dir_image}${name_image}${extension_image}
  else
    ffmpeg -i ${dir_in_video}${name_video}.${extension_video} -f image2 -vcodec mjpeg -qscale 1 -qmin 1 -qmax 1 -r ${frame_rate} ${dir_image}${name_image}${extension_image}
  fi
fi
